...
*/
			...
			if ((x & 31) == 0)
			{
				...
				if (dwTick - g_dwTickCount > 250)	// up to 4 times a second
				{
					...
					char szWorking[80];
					sprintf_s( szWorking, sizeof(szWorking), "%s Working...  (%d pixels completed)", szAnimi[g_iWorking&3], g_iTotalPixels);
					g_iWorking++;
...
void CFilter::DoTiledTint( CLayer *pLayer )
{
	...
	*/
	...
	float fColumns	= (float)(( iWidth * 1.0 ) / 16.0);
	int iColumns	= 0;
	float fRows		= (float)(( iHeight * 1.0 ) / 16.0);
	int iRows		= 0;
	float fAdj		= 0.0;
	div_t dt;
	int w			= 0;
