...
unsigned char* CWADItem::GetBits(int iMipNumber)
{
	...
	case WAD2_TYPE:
		if (IsQPic())
		{
			...
			if (m_pbyData && (m_iTotalSize >= (int)sizeof(qpic_t)))
			{
				qpic_t* pQPic = (qpic_t*)m_pbyData;
				return pQPic->data;   // pixel indices into quake1_pal
			}
			return NULL;
		}
...
void CWADList::DiscoverWidthHeight(LPBYTE pbyData, int iDataSize, char cType, LPDWORD pdwWidth, LPDWORD pdwHeight)
{
	...
	if ((cType == WAD2_TYPE_QPIC) && (GetWADType() == WAD2_TYPE))
	{
		// Quake qpic header: int width, int height, then data[]
		if (iDataSize < (int)sizeof(qpic_t))
		{
			// At minimum we want width + height + some data
			return;
		}

		qpic_t* pQPic = (qpic_t*)pbyData;

		if (pQPic->width > 0 && pQPic->height > 0)
		{
			// Optional safety: validate that lump is big enough for width*height pixels
			const int iMinSize = sizeof(int) * 2 + pQPic->width * pQPic->height;
			if (iDataSize >= iMinSize)
			{
				(*pdwWidth) = (DWORD)pQPic->width;
				(*pdwHeight) = (DWORD)pQPic->height;
			}
		}
	}
}
