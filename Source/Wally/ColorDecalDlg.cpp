...
BOOL CColorDecalDlg::VerifyHalfLifeDirectory()
{
	...
	FILE* fp = NULL;
	errno_t err = fopen_s(&fp, TrimSlashes(strDirectory) + "\\hl.exe", "r");

	if( err != 0 )
	{
		err = fopen_s(&fp, TrimSlashes(strDirectory) + "\\cstrike.exe", "r");		
		if (err != 0)
		{
			err = fopen_s(&fp, TrimSlashes(strDirectory) + "\\bshift.exe", "r");			
		}
	}	
	...
	if( err == 0 )
	{
		...
		while (bScanning)
		{
			...
			if ( fFinder.IsDirectory() && !fFinder.IsDots() )
			{
				...
				err = fopen_s( &fp, TrimSlashes( fFinder.GetFilePath()) + "\\liblist.gam", "r");
...
/* static */ void CColorDecalDlg::OnWizardColorDecal( CWallyApp *pWallyApp ) 
{
	...
	FILE* fp = NULL;
	errno_t err = fopen_s(&fp, strPath, "r");

	if (err != 0)
	{
		err = fopen_s(&fp, strPath, "wb");
		if (err == 0)
		{
